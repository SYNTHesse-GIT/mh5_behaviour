...
class WalkingBase:
    ...
    def setJointCommands(self, chain, qs, vels=None, accs=None):
        if vels is None:
            vels = [0.0] * qs.rows()
        if accs is None:
            accs = [0.0] * qs.rows()
        for i, jn in enumerate(chain.names):
    ...
    def run_standing(self):
        if self.phase_step <= self.phase_total_steps:
            for frame_iter in self.frame_iters:
                ...
                vels = self.jntArrayDiff(new_qs, frame_iter.qs, 1.0)
                self.setJointCommands(frame_iter.chain, new_qs, vels)
