...
class WalkingBase:
    ...
    def readParams(self, name_space):
        ...
        if 'using' not in self.params:
            rospy.logerror('no controllers specified in the parameters (using:)')
            raise ValueError('no controllers specified in the parameters (using:)')
        self.using_controllers = self.params['using']


    def startSubscribers(self, name):
        self.command = rospy.Subscriber(f'{name}/command', String, self.handleCommandCallback)
        self.state = rospy.Subscriber('joint_states', JointState, self.handleJointStateCallback)
    ...
    def handleJointStateCallback(self, msg):
        """Handles the `joint_states` messages by updating the `joint_states` dictionary.
        Only the joints that are present in the message are updated.
        ...
        for i, joint_name in enumerate(msg.name):
            self.joint_states[joint_name] = JS(p=msg.position[i], v=msg.velocity[i], l=msg.effort[i])
        ...
        for controller in self.available_controllers:
            if controller.name in self.using_controllers:
                if controller.state == 'running':
                    # attempting to start an already running controller will issue an error...
                    rospy.loginfo(f'controller {controller.name} already running...')
                else:
                    # attempt to start controller
                    req = SwitchControllerRequest(start_controllers=[controller.name], timeout=5, strictness=2)
                    resp = self.switch_controller(req)
                    if not resp.ok:
                        # if we failed to start one of the controllers abort and stop all
                        # already started controllers then raise and exception
                        rospy.logerr(f'failed to start controller {controller.name}')
                        self.stopControllers()
                        raise RuntimeError(f'failed to start controller {controller.name}')
                    else:
                        rospy.loginfo(f'controller {controller.name} started')
                # for a running controllers register a `command` publisher and store the
                # joint in order they are handled by controller
                pub = rospy.Publisher(f'{controller.name}/command', Float64MultiArray, queue_size=5)
                self.controllers[controller.name] = {
                    'res': controller.claimed_resources[0].resources,
                    'pub': pub
                }
    ...
    def publishCommands(self):
        ...
        """
        for c_name, c_info in self.controllers.items():
            command = Float64MultiArray(len(c_info.res))
            for index, joint_name in enumerate(c_info['res']):
                command[index] = self.joint_commands.get(joint_name, joint_states[joint_name].p)
            c_info['pub'].publish(command)
